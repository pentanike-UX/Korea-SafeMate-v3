...
export function SubpageHero({
  ...
  return (
    <section className={cn("relative overflow-hidden border-b border-border/60 bg-card/80", className)}>
      ...
      <div className="page-container relative py-11 sm:py-13 md:py-14">
        {eyebrow ? <div className="mb-3">{eyebrow}</div> : null}
        <h1 className="text-text-strong max-w-3xl text-3xl font-semibold tracking-tight text-balance sm:text-4xl">
          {title}
        </h1>
        {description ? (
          <p className="text-muted-foreground mt-4 max-w-2xl text-[15px] leading-relaxed sm:text-base">{description}</p>
        ) : null}
      </div>
    </section>
  );
