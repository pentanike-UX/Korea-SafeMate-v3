...
export default async function GuardianPostPreviewPage({ params, searchParams }: Props) {
  ...
  if (!verified || verified.postId !== postId) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <Link href="/guardian/posts" className="text-primary mt-6 inline-block text-sm font-semibold underline">
          Back to posts
        </Link>
      </div>
    );
  ...
  if (!post) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <Link href={`/guardian/posts/${postId}/edit`} className="text-primary mt-6 inline-block text-sm font-semibold underline">
          Back to editor
        </Link>
      </div>
    );
